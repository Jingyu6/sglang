...
def build_tidar_positions_and_mask_prefill(
    ...
    positions = torch.repeat_interleave(seq_lens, repeats=block_size) + offsets.repeat(bs)

    # Placeholder: user should supply planned ids; zeros for scaffold
    draft_tokens = torch.full((bs * block_size,), mask_token_id, dtype=torch.int64, device=device)
...
def build_tidar_positions_and_mask_decode(
    ...
    offsets = (
        torch.arange(block_size + 1, device=device).unsqueeze(0) + 
        torch.arange(block_size + 1, device=device).unsqueeze(1)
    ...
    masks = []
    for i in range(bs):
        prefix_mask = torch.full((B, seq_lens[i].item()), True, dtype=torch.bool, device=device)
        draft_mask = torch.zeros((B, B), dtype=torch.bool, device=device)
        # first triangular mask
        draft_mask[:block_size, :block_size] = torch.tril(torch.ones((block_size, block_size), dtype=torch.bool, device=device))
